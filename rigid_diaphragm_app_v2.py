...
def _inject_css() -> None:
    """Small CSS tweaks to make the app feel more 'report-like'."""
    st.markdown(
        """
<style>
...
/* Subtle table styling (Streamlit renders dataframes in a div table wrapper) */
div[data-testid="stDataFrame"] { border-radius: 12px; border: 1px solid rgba(49, 51, 63, 0.15); }
div[data-testid="stTable"] { border-radius: 12px; border: 1px solid rgba(49, 51, 63, 0.15); }

/* Make the primary button feel more 'app-like' */
button[kind="primary"] { border-radius: 10px; }

/* Reduce the dead-grey look by adding a light background to main area */
.main { background: linear-gradient(180deg, rgba(250,251,252,0.9), rgba(255,255,255,1)); }
</style>
        """,
...
def _show_key_metrics(summary: Dict[str, float]) -> None:
    """Display key results in a compact, professional metric layout."""
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Xcm (working)", f"{summary['Xcm']:.2f} m")
    c2.metric("Ycm (working)", f"{summary['Ycm']:.2f} m")
    c3.metric("Xcr (CoR)", f"{summary['Xcr']:.2f} m")
    c4.metric("Ycr (CoR)", f"{summary['Ycr']:.2f} m")

    c5, c6, c7, c8 = st.columns(4)
    c5.metric("ex (real)", f"{summary['ex']:.2f} m")
    c6.metric("ey (real)", f"{summary['ey']:.2f} m")
    c7.metric("Jp", f"{summary['Jp']:.3f}")
    c8.metric("Applied loads", f"Fx={summary['Fx']:.2f} kN • Fy={summary['Fy']:.2f} kN")


def _style_forces_table(df_forces: pd.DataFrame) -> "pd.io.formats.style.Styler":
    """Styler for the forces summary (zebra rows + sign coloring + in-cell bars)."""
    num_cols = [c for c in df_forces.columns if c != "Wall Name"]

    def _stripe_rows(row):
        return ["background-color: rgba(0,0,0,0.03);"] * len(row) if (row.name % 2 == 1) else [""] * len(row)

    def _force_color(v):
        if pd.isna(v):
            return ""
        return "color: #0b6b2f; font-weight: 600;" if v >= 0 else "color: #b00020; font-weight: 600;"

    styler = (
        df_forces.style
        .format(precision=2, na_rep="")
        .apply(_stripe_rows, axis=1)
        .applymap(_force_color, subset=num_cols)
    )

    # In-cell bars for totals (helps readability at a glance)
    for col in ["Vx (kN)", "Vy (kN)"]:
        if col in df_forces.columns:
            try:
                styler = styler.bar(subset=[col], align="zero")
            except Exception:
                pass
...
def _style_full_table(df_full: pd.DataFrame) -> "pd.io.formats.style.Styler":
    """Styler for the full calculation table (zebra rows + subtle emphasis)."""
    def _stripe_rows(row):
        return ["background-color: rgba(0,0,0,0.02);"] * len(row) if (row.name % 2 == 1) else [""] * len(row)
...
- Displays a table with all the calculations.
    ...
    if run:
        ...
        _show_key_metrics(summary)


        tabs = st.tabs(["Wall forces summary", "Full calculation table", "Downloads"])

        # -----------------------------
        # Tab 1 — Wall forces summary (per wall)
        # -----------------------------
        with tabs[0]:
            st.markdown(
                "<h3 style='margin-top: 0.25rem;'>Wall forces summary</h3>",
                unsafe_allow_html=True,
            )
            st.caption("Quick view of direct shear + real torsion + accidental torsion contributions, and totals per wall.")
            ...
            try:
                styler = _style_forces_table(df_forces)
                st.dataframe(styler, use_container_width=True, height=360)
            except Exception:
                st.dataframe(df_forces, use_container_width=True, height=360)

        # -----------------------------
        # Tab 2 — Full calculation table (Excel-like)
        # -----------------------------
        with tabs[1]:
            st.markdown(
                "<h3 style='margin-top: 0.25rem;'>Wall-by-wall results (full)</h3>",
                unsafe_allow_html=True,
            )
            st.caption("This is the full Excel-style table, rounded to 2 decimals for readability.")
            ...
            try:
                styler_full = _style_full_table(df_show)
                st.dataframe(
                    styler_full,
                    use_container_width=True,
                    height=620,
